function [success,V,I,Ybus,loadData,genbus,pq,idxTripGen,angVSwing] = runpfDS(casedata,V_PV,V_PQ,flag,dBus,sD,flginit,pI,qI,busnumI,simTime,genTripBus,genTripTime,mpopt,fname,solvedcase)
...
%
%   $Id: runpf.m 2644 2015-03-11 19:34:22Z ray $
%
...
%% HJ Edit - Change load to simulate disturbance
% if flag==1
% %     bus(pq(dBus),3)=sD(1);
% %     bus(pq(dBus),4)=sD(2);
%       casedata.bus(find(casedata.bus(:,1)==dBus),3)=sD(1);
%       casedata.bus(find(casedata.bus(:,1)==dBus),4)=sD(2);
% end
%% read data
...
%% HJ Edit - Trip generator and rearrange voltages
idxTripGen=find(mpc.bus(:, 1) == genTripBus);
if (genTripTime<1e10 && simTime>=genTripTime)
    mpc.gen(find(mpc.gen(:, 1) == genTripBus), GEN_STATUS)=0; 
    V_PQ=cat(1,V_PV(idxTripGen),V_PQ);
    V_PV(idxTripGen,:)=[];
...
[ref, pv, pq] = bustypes(bus, gen); % if reference bus generator is off, the reference bus will be the first PV bus that has online genberator and the PV bus list will be reduced by 1. If any other PV bus is turned off, then that bus will become part of the pq bus list. So, in order to keep the same number of buses in the return argument vectors of PV and PQ buses, we need to insert the PV or reference bus voltage that has been converted into a PQ bus into the original positon of the PV bus vector.
% in the main program, the differential equation soluton should be skipped and voltage of the now PQ bus should be assigned as that being generated by the turned off generator and returned back to the runpfDS function.
%% HJ Edit - Change load to simulate disturbance
if flag==1
    bus(pq(dBus),3)=sD(1);
    bus(pq(dBus),4)=sD(2);
...
if dc                               %% DC formulation
    ...
    if (flginit==1)% && simTime<genTripTime)
        V0([ref;pv])=V_PV;          %% HJ Edit: default initial voltages as given in the power flow case are used when power flow is run for the first time
        V0(pq)=V_PQ;                %% For subsequent iterations, voltages sent from the main program are used along with the power injection at the wind generator buses
        %  Here, when generator trip happens,the voltage from the tripped PV bus will need to be reassigned to the corresponding PQ bus as Matpower will change the PV and PQ buses
        bus(busnumI,3)=pI;
        bus(busnumI,4)=qI;
%      elseif (flginit==1 && simTime>=genTripTime)
%          V0([ref;pv])=V_PV;
%          V0(pq)=[V_PV(idxTripGen);V_PQ];
%          bus(busnumI,3)=pI;
%          bus(busnumI,4)=qI;
    end
    if qlim
    ...
    while (repeat)
        ...
        switch alg
            case 'NR'
                [V, success, iterations] = newtonpfDS(Ybus, Sbus, V0, ref, pv, pq, mpopt,busnumI);
            case {'FDXB', 'FDBX'}
...
%% Modifications to Pass Voltage and Current Phasors at the PV terminals to The Dynamics Simulation Program; also to pass load data to the program.
if (simTime<genTripTime)
    V_PV=V([ref;pv]);
    V_PQ=V(pq);
    %I_PV=Ybus([ref;pv],:)*V;
    I=Ybus*V;
    ...
    if (idxTripGen==find(V(ref)))
        V_PV=[V_PQ(1);V(ref);V(pv)];
        genbus=[pq(1);ref;pv];
    else
        V_PV=[V(ref);V(pv(pv<idxTripGen));V(pq(1));V(pv(pv>idxTripGen))];
        genbus=[ref;pv(pv<idxTripGen);idxTripGen;pv((pv>idxTripGen))];
    end
    V_PQ=V(pq(2:end));
    I=Ybus*V;
    loadData=bus(pq(2:end),3:4);
    pq=pq(2:end,1);
end
angVSwing=angle(V(ref));
end
